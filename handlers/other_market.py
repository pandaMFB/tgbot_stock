...
@dp.message_handler(lambda message: message.text and 'Other' in  message.text, state=None)
async def other_market_info(message: types.Message):
    await FSMAdmin.other_market.set()
    await message.reply("\U000023F3List of available other market:"
                        "\nNAME : INDEX"
                        "\nAluminum ; AL3M"
                        "\nGold ; Gold"
                        "\nCopper/Med' ; Copper3M"
...
@dp.message_handler(content_types=['text'], state=FSMAdmin.other_market)
async def get_other_market_info(message: types.Message, state: FSMContext):
    try:
        with open('files_info/other_market_index.txt', 'r') as file:
            for line in file:
                if message.text.lower() in line.lower():
                    index_name = line.split(';')[0]
                    index_id = line.split(';')[1]
        ...
        if message.text != '/Exit':
            ...
            if current_state is None:
                return
            await state.finish()
            await message.reply("\U0001F6B8Ok, You are out of the section Other market",
                                reply_markup=nav.indexMenu)


def register_handlers_other_market(dp: Dispatcher):
    dp.register_message_handler(other_market_info, lambda message: message.text and 'All list' in message.text)
    dp.register_message_handler(get_other_market_info, Text(equals='Exit', ignore_case=True), state="*")
    dp.register_message_handler(get_other_market_info, content_types=['text'], state=FSMAdmin.other_market)
